...
class Feed extends Component {
  ...
  componentDidMount() {
    fetch('http://localhost:8090/status/'+this.props.userId,{
      headers:{
  ...
  loadPosts = direction => {
    ...
    fetch('http://localhost:8090/feeds?page='+page,{
      headers:{
        Authorization:'Bearer: '+this.props.token
      }
    })
      ...
      .then(resData => {
        console.log('resData---',resData.posts[0].id)
        this.setState({
  ...
  finishEditHandler = postData => {
    ...
    let url = 'http://localhost:8090/feed';
    let method = 'POST'
    if (this.state.editPost) {
      url = 'http://localhost:8090/feeds/'+this.state.editPost.id;
      method = 'PUT'
    }
  ...
  deletePostHandler = postId => {
    ...
    fetch('http://localhost:8090/feeds/'+postId,
    {headers:{
      Authorization:'Bearer: '+this.props.token
    }})
      .then(res => {
